...
GO
CREATE PROCEDURE [dbo].[sphrmNewEmployeesSelectListSearch]
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
  @ActiveState Bit,
 ...
 WHERE  
   ...
   AND ( 
     ...
     OR LOWER(ISNULL([HRM_Departments11].[DepartmentID],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([HRM_Departments11].[Description],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([HRM_Designations14].[Description],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([HRM_Designations14].[ShortName],'')) LIKE @KeyWord1
     OR STR(ISNULL([HRM_Designations14].[Sequence], 0)) LIKE @KeyWord1
     OR LOWER(ISNULL([HRM_Divisions16].[DivisionID],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([HRM_Divisions16].[Description],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([HRM_Offices25].[Description],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([HRM_Offices25].[Address],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([HRM_Offices25].[City],'')) LIKE @KeyWord1
   ) 
  ORDER BY
