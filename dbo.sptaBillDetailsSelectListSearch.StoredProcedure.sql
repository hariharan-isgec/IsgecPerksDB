...
GO
CREATE PROCEDURE [dbo].[sptaBillDetailsSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 WHERE  
   ( 
     ...
     OR STR(ISNULL([TA_BillDetails].[AmountRateOU], 0)) LIKE @KeyWord1
     OR STR(ISNULL([TA_BillDetails].[AmountTaxOU], 0)) LIKE @KeyWord1
     OR STR(ISNULL([TA_BillDetails].[ConversionFactorOU], 0)) LIKE @KeyWord1
     OR LOWER(ISNULL([TA_BillDetails].[CurrencyMain],'')) LIKE @KeyWord1
   ) 
  ORDER BY
     ...
     CASE @OrderBy WHEN 'AmountRateOU' THEN [TA_BillDetails].[AmountRateOU] END,
     CASE @OrderBy WHEN 'AmountRateOU DESC' THEN [TA_BillDetails].[AmountRateOU] END DESC,
     CASE @OrderBy WHEN 'AmountTaxOU' THEN [TA_BillDetails].[AmountTaxOU] END,
     CASE @OrderBy WHEN 'AmountTaxOU DESC' THEN [TA_BillDetails].[AmountTaxOU] END DESC,
     CASE @OrderBy WHEN 'ConversionFactorOU' THEN [TA_BillDetails].[ConversionFactorOU] END,
     CASE @OrderBy WHEN 'ConversionFactorOU DESC' THEN [TA_BillDetails].[ConversionFactorOU] END DESC,
     CASE @OrderBy WHEN 'CurrencyMain' THEN [TA_BillDetails].[CurrencyMain] END,
     CASE @OrderBy WHEN 'CurrencyMain DESC' THEN [TA_BillDetails].[CurrencyMain] END DESC,
