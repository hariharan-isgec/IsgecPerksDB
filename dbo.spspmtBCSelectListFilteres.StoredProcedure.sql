...
GO
CREATE Procedure [dbo].[spspmtBCSelectListFilteres]
  @Filter_RequestedBy NVarChar(8),
  @Filter_VendorID NVarChar(6),
  @Filter_ToPlaceID NVarChar(30),
  @StartRowIndex int,
  @MaximumRows int,
  @OrderBy NVarChar(50),
  @LoginID NVarChar(8),
 ...
 ,BCID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [SPMT_BC].[CreatedBy] = ''' + @LoginID + ''''    
  IF (@Filter_RequestedBy > '') 
    SET @LGSQL = @LGSQL + ' AND [SPMT_BC].[RequestedBy] = ''' + @Filter_RequestedBy + ''''
  IF (@Filter_VendorID > '') 
    SET @LGSQL = @LGSQL + ' AND [SPMT_BC].[VendorID] = ''' + @Filter_VendorID + ''''
  IF (@Filter_ToPlaceID > '') 
    SET @LGSQL = @LGSQL + ' AND [SPMT_BC].[ToPlaceID] = ''' + @Filter_ToPlaceID + ''''
