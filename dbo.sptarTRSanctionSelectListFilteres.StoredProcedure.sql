...
GO
CREATE PROCEDURE [dbo].[sptarTRSanctionSelectListFilteres]
 ...
 ,RequestID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [TA_TravelRequest].[StatusID] >= 8 '
  IF (@LoginID<>'0340') 
    SET @LGSQL = @LGSQL + ' AND ([TA_TravelRequest].[BHApprovalBy] = ''' + @LoginID + ''' OR [TA_TravelRequest].[RequestedFor] IN (Select CardNo From HRM_Employees where TASanctioningAuthority = ''' + @LoginID + '''))'
  IF (@Filter_ProjectID > '') 
