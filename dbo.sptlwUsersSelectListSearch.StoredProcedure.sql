...
GO
CREATE PROCEDURE [dbo].[sptlwUsersSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 WHERE  
   ( 
     ...
     OR LOWER(ISNULL([aspnet_Users].[wp_user],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([aspnet_Users].[pw],'')) LIKE @KeyWord1
   ) 
  ORDER BY
     ...
     CASE @OrderBy WHEN 'wp_user' THEN [aspnet_Users].[wp_user] END,
     CASE @OrderBy WHEN 'wp_user DESC' THEN [aspnet_Users].[wp_user] END DESC,
     CASE @OrderBy WHEN 'pw' THEN [aspnet_Users].[pw] END,
     CASE @OrderBy WHEN 'pw DESC' THEN [aspnet_Users].[pw] END DESC,
