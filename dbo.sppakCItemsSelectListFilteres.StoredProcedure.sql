...
GO
CREATE PROCEDURE [dbo].[sppakCItemsSelectListFilteres]
  @Filter_RootItem Int,
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
 ...
 ,RootItem Int NOT NULL
 ,ItemNo Int NOT NULL
  )
  SET @LGSQL = 'INSERT INTO #PageIndex (' 
  SET @LGSQL = @LGSQL + 'RootItem'
  SET @LGSQL = @LGSQL + ', ItemNo'
  SET @LGSQL = @LGSQL + ')'
  SET @LGSQL = @LGSQL + ' SELECT '
  SET @LGSQL = @LGSQL + '[PAK_Items].[RootItem]'
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
                        ...
                        ELSE '[PAK_Items].[RootItem],[PAK_Items].[ItemNo]'
                    END
  EXEC (@LGSQL)
  ...
  FROM [PAK_Items] 
      INNER JOIN #PageIndex
          ON [PAK_Items].[RootItem] = #PageIndex.RootItem
          AND [PAK_Items].[ItemNo] = #PageIndex.ItemNo
