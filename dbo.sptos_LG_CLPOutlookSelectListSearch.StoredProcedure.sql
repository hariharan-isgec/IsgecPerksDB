...
GO
CREATE PROCEDURE [dbo].[sptos_LG_CLPOutlookSelectListSearch]
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
  @LoginID NVarChar(9),
 ...
 WHERE  [TOS_CLPData].[ProjectID] IN (SELECT PROJECTID FROM TOS_USERVIEWS WHERE CLPVIEW = 1 AND USERID=@LoginID) 
   AND ( 
