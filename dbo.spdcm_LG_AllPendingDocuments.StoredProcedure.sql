...
GO
CREATE PROCEDURE [dbo].[spdcm_LG_AllPendingDocuments]
  @FromProjectID NVarChar(6),
  @ToProjectID NVarChar(6), 
  ...
  WHERE
        [DCM_CustomerTmtlDetails].[ProjectID] >= @FromProjectID 
    AND [DCM_CustomerTmtlDetails].[ProjectID] <= @ToProjectID 
    AND [DCM_CustomerTmtlDetails].[TempDetailStatus] = 1 
  ORDER BY  
     [DCM_CustomerTmtlDetails].[ProjectID]
