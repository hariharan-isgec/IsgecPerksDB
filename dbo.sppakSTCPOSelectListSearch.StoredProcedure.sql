...
GO
CREATE PROCEDURE [dbo].[sppakSTCPOSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 WHERE  1 = 
		case when (@DivisionID > 0) then 
			case when [PAK_PO].[DivisionID] = (@DivisionID) then 1 else 0 end
		  else
			1
		end 
   AND [PAK_PO].[POFOR] = 'TC'  
   AND 1 = 
        case when (substring(@LoginID,1,2)='UP') then
          case when (SUBSTRING([PAK_PO].[SupplierID],2,8) = @LoginID) then 1 else 0 end
		else
			1
		end    
   AND ( 
