...
GO
CREATE PROCEDURE [dbo].[sptosTasksSelectListSearch]
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
  @OrderBy NVarChar(50),
 ...
 ,TaskID Int NOT NULL
  ...
  INSERT INTO #PageIndex (TaskID)
  SELECT [TOS_Tasks].[TaskID] FROM [TOS_Tasks]
 ...
 WHERE  
   ( 
         STR(ISNULL([TOS_Tasks].[TaskID], 0)) LIKE @KeyWord1
     OR LOWER(ISNULL([TOS_Tasks].[Description],'')) LIKE @KeyWord1
     OR STR(ISNULL([TOS_Tasks].[wuDefined], 0)) LIKE @KeyWord1
     OR STR(ISNULL([TOS_Tasks].[Sequence], 0)) LIKE @KeyWord1
     OR LOWER(ISNULL([TOS_Tasks].[ProjectID],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([TOS_Tasks].[ElementID],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([TOS_Tasks].[FunctionalAreaID],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([TOS_Tasks].[TaskTypeID],'')) LIKE @KeyWord1
     OR STR(ISNULL([TOS_Tasks].[wuEntered], 0)) LIKE @KeyWord1
     OR STR(ISNULL([TOS_Tasks].[wuBalance], 0)) LIKE @KeyWord1
     OR STR(ISNULL([TOS_Tasks].[wuNoteID], 0)) LIKE @KeyWord1
     OR LOWER(ISNULL([TOS_Tasks].[AssignedTo],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([TOS_Tasks].[Predecessors],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([TOS_Tasks].[Successors],'')) LIKE @KeyWord1
     OR STR(ISNULL([TOS_Tasks].[ParentTaskID], 0)) LIKE @KeyWord1
   ) 
  ORDER BY
