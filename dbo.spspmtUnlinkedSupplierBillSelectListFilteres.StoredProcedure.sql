...
GO
CREATE PROCEDURE [dbo].[spspmtUnlinkedSupplierBillSelectListFilteres]
  @Filter_TranTypeID NVarChar(3),
  @Filter_BPID NVarChar(9),
  @Filter_SupplierName NVarChar(100),
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @OrderBy NVarChar(50),
 ...
 ,IRNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [SPMT_SupplierBill].[LogisticLinked] = 0 '
  IF (@Filter_TranTypeID > '') 
    SET @LGSQL = @LGSQL + ' AND [SPMT_SupplierBill].[TranTypeID] = ''' + @Filter_TranTypeID + ''''
  IF (@Filter_BPID > '') 
    SET @LGSQL = @LGSQL + ' AND [SPMT_SupplierBill].[BPID] = ''' + @Filter_BPID + ''''
  IF (@Filter_SupplierName > '' AND @Filter_BPID = '') 
    SET @LGSQL = @LGSQL + ' AND [SPMT_SupplierBill].[SupplierName] = ''' + @Filter_SupplierName + ''''
