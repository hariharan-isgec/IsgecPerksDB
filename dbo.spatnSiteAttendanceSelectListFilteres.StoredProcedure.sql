...
GO
CREATE PROCEDURE [dbo].[spatnSiteAttendanceSelectListFilteres]
  @Filter_MonthID Int,
 ...
 ,CardNo NVarChar(8) NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [ATN_SiteAttendance].[ATNStatusID] = 8 '
  IF (@LoginID <> '0340') 
    SET @LGSQL = @LGSQL + '  AND [ATN_SiteAttendance].[CardNo] IN (select cardno from hrm_employees where verifierid = ''' + @LoginID + '''   ) '
