...
GO
CREATE PROCEDURE [dbo].[sppakSTCPOSelectListFilteres]
  @Filter_POTypeID Int,
 ...
 ,SerialNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [PAK_PO].[POFOR] = ''' + 'TC' + ''''   
  if(substring(@LoginID,1,2)='UP')
	SET @LGSQL = @LGSQL + '  AND  SUBSTRING([PAK_PO].[SupplierID],2,8) = ''' + @LoginID + ''''  
  IF (@DivisionID > 0) 
	SET @LGSQL = @LGSQL + ' AND [PAK_PO].[DivisionID] = ''' + STR(@DivisionID) + '''' 
  IF (@Filter_POTypeID > 0) 
    SET @LGSQL = @LGSQL + ' AND [PAK_PO].[POTypeID] = ' + STR(@Filter_POTypeID)
  IF (@Filter_SupplierID > '') 
    SET @LGSQL = @LGSQL + ' AND [PAK_PO].[SupplierID] = ''' + @Filter_SupplierID + ''''
  IF (@Filter_ProjectID > '') 
  ...
  IF (@Filter_POStatusID > 0) 
    SET @LGSQL = @LGSQL + ' AND [PAK_PO].[POStatusID] = ' + STR(@Filter_POStatusID)
  IF (@Filter_IssuedBy > '') 
