...
GO
CREATE PROCEDURE [dbo].[spijt_LG_AttendanceSelectListFilteresHR]
  @Filter_CardNo NVarChar(8),
  @Filter_C_ProjectSiteID NVarChar(6),
  @Filter_DataMonth Int,
  @Filter_VerificationStatus NVarChar(3),
  @StartRowIndex int,
  @MaximumRows int,
  @FinYear NVarChar(4),
  @OrderBy NVarChar(50),
 ...
 ,RecordID NVarChar(15) NOT NULL
  ...
  IF (@Filter_C_ProjectSiteID > '') 
    SET @LGSQL = @LGSQL + ' AND [ATNv_SiteAttendance].[C_ProjectSiteID] = ''' + @Filter_C_ProjectSiteID + ''''
  IF (@Filter_DataMonth > 0) 
    SET @LGSQL = @LGSQL + ' AND [ATNv_SiteAttendance].[DataMonth] = ' + STR(@Filter_DataMonth)
  IF (@Filter_VerificationStatus > '') 
    SET @LGSQL = @LGSQL + ' AND [ATNv_SiteAttendance].[UnVerified] = ' + @Filter_VerificationStatus 
