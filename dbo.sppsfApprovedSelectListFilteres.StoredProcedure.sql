...
GO
CREATE PROCEDURE [dbo].[sppsfApprovedSelectListFilteres]
  @Filter_SerialNo Int,
 ...
 ,SerialNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + ' AND [PSF_HSBCMain].[PSFStatus] = ''' + STR(@PSFStatus) + '''' 
  ...
  IF (@Filter_CreatedBy > '') 
    SET @LGSQL = @LGSQL + ' AND [PSF_HSBCMain].[CreatedBy] = ''' + @Filter_CreatedBy + ''''
  IF (@Filter_ApprovedBy > '') 
    SET @LGSQL = @LGSQL + ' AND [PSF_HSBCMain].[ApprovedBy] = ''' + @Filter_ApprovedBy + ''''
