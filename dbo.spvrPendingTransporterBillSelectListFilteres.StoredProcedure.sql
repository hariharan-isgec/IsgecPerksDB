...
GO
CREATE PROCEDURE [dbo].[spvrPendingTransporterBillSelectListFilteres]
  @Filter_TransporterID NVarChar(9),
  @Filter_ForwardedBy NVarChar(8),
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @BillStatusID Int,
 ...
 ,IRNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + ' AND [VR_TransporterBill].[BillStatusID] = ''' + STR(@BillStatusID) + '''' 
  IF (@Filter_TransporterID > '') 
    SET @LGSQL = @LGSQL + ' AND [VR_TransporterBill].[TransporterID] = ''' + @Filter_TransporterID + ''''
  IF (@Filter_ForwardedBy > '') 
    SET @LGSQL = @LGSQL + ' AND [VR_TransporterBill].[ForwardedBy] = ''' + @Filter_ForwardedBy + ''''
