...
GO
CREATE PROCEDURE [dbo].[sppakSiteItemMasterAutoCompleteList]
  @LoginID NVarChar(8),
  @Prefix NVarChar(250),
  @Records Int,
  @ByCode Int 
  ...
  SET @LGSQL = @LGSQL + ' AND (LOWER(ISNULL([PAK_SiteItemMaster].[ProjectID],'''')) LIKE ''' + @Prefix1 + ''''
  SET @LGSQL = @LGSQL + ' OR LOWER(ISNULL([PAK_SiteItemMaster].[SiteMarkNo],'''')) LIKE ''' + @Prefix1 + ''''
