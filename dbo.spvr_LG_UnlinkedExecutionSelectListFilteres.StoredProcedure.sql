...
GO
CREATE PROCEDURE [dbo].[spvr_LG_UnlinkedExecutionSelectListFilteres]
...
--Request Sates 10-ODCApproved,13-Self Approved,22-Delayed Approved, 25-Detention Approved
  SET @LGSQL = @LGSQL + '  WHERE [VR_RequestExecution].[RequestStatusID] IN (10,13,22,25) '
  SET @LGSQL = @LGSQL + '  AND   [VR_RequestExecution].[IRNO] IS NULL '
--If Linked Then Only Main
  SET @LGSQL = @LGSQL + '  AND   ([VR_RequestExecution].[Linked] = 0 OR ([VR_RequestExecution].[Linked]=1 AND [VR_RequestExecution].[SRNNo]=[VR_RequestExecution].[LinkID] ) ) '
