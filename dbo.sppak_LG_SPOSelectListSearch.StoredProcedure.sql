...
GO
CREATE PROCEDURE [dbo].[sppak_LG_SPOSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 WHERE  [PAK_PO].[SupplierID] = case  when SUBSTRING(@SupplierID,1,3) = 'SUP' then @SupplierID else [PAK_PO].[SupplierID] end
    AND [PAK_PO].[POFOR] = 'PK'
    AND [PAK_PO].[POStatusID] IN (2,3,4,5,6)
   AND ( 
