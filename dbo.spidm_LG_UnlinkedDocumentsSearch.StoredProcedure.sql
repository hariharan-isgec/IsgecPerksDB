...
GO
CREATE PROCEDURE [dbo].[spidm_LG_UnlinkedDocumentsSearch]
  @Filter_TmtlID Int,
  ...
  BEGIN
   ...
   WHERE [IDM_Documents].[ReceivedLocked] = 0 
    AND (([IDM_Documents].[VendorDocument] = 0) OR ([IDM_Documents].[VendorDocument] = 1 AND [IDM_Documents].[VDReceived] = 1)) 
    AND ([IDM_Documents].[RevisionNo] = (SELECT MAX(RevisionNo) AS TEMP1 FROM IDM_Documents AS B WHERE ([B].[ProjectID] = [IDM_Documents].[ProjectID]) AND ([B].[DocumentID] = [IDM_Documents].[DocumentID]))) 
    AND [IDM_Documents].[cmba] NOT IN (SELECT cmba FROM [IDM_TransmittalDetail] WHERE [IDM_TransmittalDetail].[TmtlID] = @Filter_TmtlID) 
    AND [IDM_Documents].[ProjectID] = @Filter_ProjectID  
    AND( 
