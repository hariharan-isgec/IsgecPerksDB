...
GO
CREATE PROCEDURE [dbo].[sptosOTPTasksAutoCompleteList]
  ...
  SET @LGSQL = @LGSQL + ' AND (LOWER(ISNULL([TOS_Tasks].[ProjectID],'''')) LIKE ''' + @Prefix1 + ''''
  SET @LGSQL = @LGSQL + ' OR LTRIM(STR(ISNULL([TOS_Tasks].[TaskID],0))) LIKE ''' + @Prefix1 + ''''
