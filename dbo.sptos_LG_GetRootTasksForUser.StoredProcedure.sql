...
GO
CREATE PROCEDURE [dbo].[sptos_LG_GetRootTasksForUser]
  @LoginID NVarChar(8),
  ...
  WHERE [TOS_Tasks].[TaskID] IN (SELECT  TOS_Projects.RootTaskID FROM TOS_Projects INNER JOIN TOS_UserViews 
                                         ON TOS_Projects.ProjectID = TOS_UserViews.ProjectID
                                 WHERE TOS_USERVIEWS.USERID = @LOGINID
                                   AND TOS_USERVIEWS.OTPVIEW = 1 
                                   AND TOS_PROJECTS.ACTIVE = 1)  
