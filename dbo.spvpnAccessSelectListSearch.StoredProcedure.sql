...
GO
CREATE Procedure [dbo].[spvpnAccessSelectListSearch]
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
  @Removed Bit,
 ...
 WHERE  
      [VPN_ConnectivityAccess].[Removed] = (@Removed)
   AND ( 
     ...
     OR LOWER(ISNULL([VPN_ConnectivityAccess].[BaaNPW],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([VPN_ConnectivityAccess].[PLMpw],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([VPN_ConnectivityAccess].[EMailPW],'')) LIKE @KeyWord1
   ) 
  ORDER BY
