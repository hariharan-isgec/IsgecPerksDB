...
GO
CREATE PROCEDURE [dbo].[sppmtChequeReceiptsSelectListFilteres]
 ...
 ,CQRNo Int NOT NULL
  ...
  IF (@Filter_VendorID > '') 
    SET @LGSQL = @LGSQL + ' AND [PMT_CQReceipts].[VendorID] = ''' + @Filter_VendorID + ''''
  IF (@Filter_StatusID > 0) 
    SET @LGSQL = @LGSQL + ' AND [PMT_CQReceipts].[StatusID] = ' + STR(@Filter_StatusID)
