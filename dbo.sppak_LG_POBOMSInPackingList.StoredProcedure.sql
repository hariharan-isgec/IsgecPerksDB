...
GO
Create PROCEDURE [dbo].[sppak_LG_POBOMSInPackingList]
  @SerialNo Int,
  @LoginID NVarChar(8),
  @PkgNo int,
  ...
  WHERE [PAK_POBOM].[SerialNo] = @SerialNo 
    AND [PAK_POBOM].[BOMNo] in (select BOMNO from PAK_PkgListH where serialNo=@SerialNo and PkgNo=@PkgNo)
  ORDER BY [PAK_POBOM].[BOMNo]
