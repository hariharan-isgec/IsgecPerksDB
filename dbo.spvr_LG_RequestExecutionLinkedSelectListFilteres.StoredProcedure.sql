...
GO
CREATE PROCEDURE [dbo].[spvr_LG_RequestExecutionLinkedSelectListFilteres]
  @Filter_LinkID Int,
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
 ...
 ,SRNNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [VR_RequestExecution].[Linked] = 1 '
  SET @LGSQL = @LGSQL + '  AND [VR_RequestExecution].[SRNNo] <> [VR_RequestExecution].[LinkID] '
  SET @LGSQL = @LGSQL + ' AND [VR_RequestExecution].[LinkID] = ' + STR(@Filter_LinkID)
