...
GO
CREATE Procedure [dbo].[speitl_LG_ImportPOListSelectListFilteres]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @OrderBy NVarChar(50),
  ...
  AS
  BEGIN
  DECLARE @LGSQL VarChar(8000), @UserDiv VarChar(10)
...
-- PO Status Free
-- User's Devisions PO Only
-- We Can Have Filter on Buyer
  SET @LGSQL = @LGSQL + '  WHERE [EITL_POList].[POStatusID] = 1 '
  SET @LGSQL = @LGSQL + '  AND   [EITL_POList].[DivisionID] = ''' + @UserDiv + '''  ' 
--SET @LGSQL = @LGSQL + '  AND   [EITL_POList].[BuyerID] = ''' + @LoginID + '''  ' 
