...
GO
CREATE Procedure [dbo].[spspmt_LG_PaymentAdviceSelectListFilteres]
  @Filter_TranTypeID NVarChar(3),
  @Filter_VendorID NVarChar(6),
  @Filter_AdviceStatusID Int,
  @StartRowIndex int,
  @MaximumRows int,
  @OrderBy NVarChar(50),
  @LoginID NVarChar(8), 
 ...
 ,AdviceNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [SPMT_TranTypes4].[GroupID] IN (SELECT GroupID FROM SYS_GroupLogins WHERE LoginID = ' + @LoginID + ') '
  ...
  IF (@Filter_AdviceStatusID > 0) 
    SET @LGSQL = @LGSQL + ' AND [SPMT_PaymentAdvice].[AdviceStatusID] = ' + STR(@Filter_AdviceStatusID)
