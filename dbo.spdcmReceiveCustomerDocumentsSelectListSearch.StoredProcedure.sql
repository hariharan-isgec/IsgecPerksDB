...
GO
CREATE PROCEDURE [dbo].[spdcmReceiveCustomerDocumentsSelectListSearch]
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 WHERE  
      [DCM_CustomerTmtlDetails].[TempDetailStatus] = @TempDetailStatus
   AND ( 
     ...
     OR STR(ISNULL([DCM_CustomerTmtlDetails].[TempDetailStatus], 0)) LIKE @KeyWord1
  ...
  ORDER BY
     ...
     CASE @orderBy WHEN 'TempDetailStatus' THEN [DCM_CustomerTmtlDetails].[TempDetailStatus] END,
     CASE @orderBy WHEN 'TempDetailStatus DESC' THEN [DCM_CustomerTmtlDetails].[TempDetailStatus] END DESC,
