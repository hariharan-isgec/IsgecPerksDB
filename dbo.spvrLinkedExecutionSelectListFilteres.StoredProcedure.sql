...
GO
CREATE PROCEDURE [dbo].[spvrLinkedExecutionSelectListFilteres]
  @Filter_IRNo Int,
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
 ...
 ,SRNNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  LEFT OUTER JOIN [VR_LinkExecutions] AS [VR_LinkExecutions15]'
  SET @LGSQL = @LGSQL + '    ON [VR_RequestExecution].[LinkID] = [VR_LinkExecutions15].[LinkID]'
  SET @LGSQL = @LGSQL + '    AND [VR_RequestExecution].[SRNNo] = [VR_LinkExecutions15].[SRNNo]'
  SET @LGSQL = @LGSQL + '  LEFT OUTER JOIN [VR_ODCReasons] AS [VR_ODCReasons16]'
  SET @LGSQL = @LGSQL + '    ON [VR_RequestExecution].[ODCReasonID] = [VR_ODCReasons16].[ReasonID]'
  SET @LGSQL = @LGSQL + '  WHERE 1 = 1 '
  IF (@Filter_IRNo > 0) 
    SET @LGSQL = @LGSQL + ' AND [VR_RequestExecution].[IRNo] = ' + STR(@Filter_IRNo)
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
                        ...
                        WHEN 'ODCByRequest' THEN '[VR_RequestExecution].[ODCByRequest]'
                        WHEN 'ODCByRequest DESC' THEN '[VR_RequestExecution].[ODCByRequest] DESC'
                        WHEN 'Linked' THEN '[VR_RequestExecution].[Linked]'
                        WHEN 'Linked DESC' THEN '[VR_RequestExecution].[Linked] DESC'
                        WHEN 'LinkID' THEN '[VR_RequestExecution].[LinkID]'
                        WHEN 'LinkID DESC' THEN '[VR_RequestExecution].[LinkID] DESC'
                        WHEN 'VehicleNotPlaced' THEN '[VR_RequestExecution].[VehicleNotPlaced]'
                        WHEN 'VehicleNotPlaced DESC' THEN '[VR_RequestExecution].[VehicleNotPlaced] DESC'
                        WHEN 'DelayedPlacement' THEN '[VR_RequestExecution].[DelayedPlacement]'
                        WHEN 'DelayedPlacement DESC' THEN '[VR_RequestExecution].[DelayedPlacement] DESC'
                        WHEN 'EmptyReturn' THEN '[VR_RequestExecution].[EmptyReturn]'
                        WHEN 'EmptyReturn DESC' THEN '[VR_RequestExecution].[EmptyReturn] DESC'
                        WHEN 'DetentionAtSupplier' THEN '[VR_RequestExecution].[DetentionAtSupplier]'
                        WHEN 'DetentionAtSupplier DESC' THEN '[VR_RequestExecution].[DetentionAtSupplier] DESC'
                        WHEN 'DebitToTransporter' THEN '[VR_RequestExecution].[DebitToTransporter]'
                        WHEN 'DebitToTransporter DESC' THEN '[VR_RequestExecution].[DebitToTransporter] DESC'
                        WHEN 'PayableToTransporter' THEN '[VR_RequestExecution].[PayableToTransporter]'
                        WHEN 'PayableToTransporter DESC' THEN '[VR_RequestExecution].[PayableToTransporter] DESC'
                        WHEN 'BorneByISGEC' THEN '[VR_RequestExecution].[BorneByISGEC]'
                        WHEN 'BorneByISGEC DESC' THEN '[VR_RequestExecution].[BorneByISGEC] DESC'
                        WHEN 'BorneBySupplier' THEN '[VR_RequestExecution].[BorneBySupplier]'
                        WHEN 'BorneBySupplier DESC' THEN '[VR_RequestExecution].[BorneBySupplier] DESC'
                        WHEN 'DebitAmount' THEN '[VR_RequestExecution].[DebitAmount]'
                        WHEN 'DebitAmount DESC' THEN '[VR_RequestExecution].[DebitAmount] DESC'
                        WHEN 'PayableAmount' THEN '[VR_RequestExecution].[PayableAmount]'
                        WHEN 'PayableAmount DESC' THEN '[VR_RequestExecution].[PayableAmount] DESC'
                        WHEN 'ISGECAmount' THEN '[VR_RequestExecution].[ISGECAmount]'
                        WHEN 'ISGECAmount DESC' THEN '[VR_RequestExecution].[ISGECAmount] DESC'
                        WHEN 'SupplierAmount' THEN '[VR_RequestExecution].[SupplierAmount]'
                        WHEN 'SupplierAmount DESC' THEN '[VR_RequestExecution].[SupplierAmount] DESC'
                        WHEN 'EmptyReturnRemarks' THEN '[VR_RequestExecution].[EmptyReturnRemarks]'
                        WHEN 'EmptyReturnRemarks DESC' THEN '[VR_RequestExecution].[EmptyReturnRemarks] DESC'
                        WHEN 'ODCReasonID' THEN '[VR_RequestExecution].[ODCReasonID]'
                        WHEN 'ODCReasonID DESC' THEN '[VR_RequestExecution].[ODCReasonID] DESC'
                        ...
                        WHEN 'VR_ODCReasons16_Description' THEN '[VR_ODCReasons16].[Description]'
                        WHEN 'VR_ODCReasons16_Description DESC' THEN '[VR_ODCReasons16].[Description] DESC'
  ...
  SELECT
		...
		[VR_RequestExecution].[MaterialSize] ,
		[VR_RequestExecution].[SizeUnit] ,
		[VR_RequestExecution].[Height] ,
		[VR_RequestExecution].[Width] ,
		[VR_RequestExecution].[Length] ,
		[VR_RequestExecution].[MaterialWeight] ,
		[VR_RequestExecution].[WeightUnit] ,
		[VR_RequestExecution].[NoOfPackages] ,
		[VR_RequestExecution].[OverDimentionConsignement] ,
		[VR_RequestExecution].[GRRemarks] ,
		[VR_RequestExecution].[TransRemarks] ,
		[VR_RequestExecution].[ReachedAtSupplierOn] ,
		[VR_RequestExecution].[ODCByRequest] ,
		[VR_RequestExecution].[Linked] ,
		[VR_RequestExecution].[LinkID] ,
		[VR_RequestExecution].[VehicleNotPlaced] ,
		[VR_RequestExecution].[DelayedPlacement] ,
		[VR_RequestExecution].[EmptyReturn] ,
		[VR_RequestExecution].[DetentionAtSupplier] ,
		[VR_RequestExecution].[DebitToTransporter] ,
		[VR_RequestExecution].[PayableToTransporter] ,
		[VR_RequestExecution].[BorneByISGEC] ,
		[VR_RequestExecution].[BorneBySupplier] ,
		[VR_RequestExecution].[DebitAmount] ,
		[VR_RequestExecution].[PayableAmount] ,
		[VR_RequestExecution].[ISGECAmount] ,
		[VR_RequestExecution].[SupplierAmount] ,
		[VR_RequestExecution].[EmptyReturnRemarks] ,
		[VR_RequestExecution].[ODCReasonID] ,
