...
GO
CREATE PROCEDURE [dbo].[sppak_LG_HPendingAtPortSelectListFilteres]
  @Filter_SerialNo Int,
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
 ...
 ,PkgNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  AND [PAK_PO2].[PortID] IN (SELECT PortID from PAK_UserPort Where UserID =''' + @LoginID + ''' ) '
  SET @LGSQL = @LGSQL + '  AND [PAK_PkgListH].[StatusID] = 5 ' --Under Receive At Port
