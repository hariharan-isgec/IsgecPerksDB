...
GO
CREATE PROCEDURE [dbo].[sptos_LG_CLPOutlookSelectListFilteres]
 ...
 ,ProgressID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [TOS_CLPData].[ProjectID] IN (SELECT PROJECTID FROM TOS_USERVIEWS WHERE CLPVIEW = 1 AND USERID=''' + @LoginID + ''')'
  IF (@Filter_ProjectID > '') 
