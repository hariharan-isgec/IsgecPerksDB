...
GO
CREATE PROCEDURE [dbo].[spnprkUserClaimsSelectListFilteres]
  @Filter_ClaimStatusID Int,
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @CardNo NVarChar(8),
 ...
 ,ClaimID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  LEFT OUTER JOIN [PRK_FinYears] AS [PRK_FinYears5]'
  SET @LGSQL = @LGSQL + '    ON [PRK_UserClaims].[FinYear] = [PRK_FinYears5].[FinYear]'
  SET @LGSQL = @LGSQL + '  WHERE 1 = 1 '
  SET @LGSQL = @LGSQL + ' AND [PRK_UserClaims].[CardNo] = (''' + @CardNo + ''')'
  SET @LGSQL = @LGSQL + ' AND [PRK_UserClaims].[FinYear] = ''' + STR(@FinYear) + '''' 
  IF (@Filter_ClaimStatusID > 0) 
    SET @LGSQL = @LGSQL + ' AND [PRK_UserClaims].[ClaimStatusID] = ' + STR(@Filter_ClaimStatusID)
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
                        ...
                        WHEN 'ClaimRefNo' THEN '[PRK_UserClaims].[ClaimRefNo]'
                        WHEN 'ClaimRefNo DESC' THEN '[PRK_UserClaims].[ClaimRefNo] DESC'
                        WHEN 'FinYear' THEN '[PRK_UserClaims].[FinYear]'
                        WHEN 'FinYear DESC' THEN '[PRK_UserClaims].[FinYear] DESC'
                        WHEN 'forRef' THEN '[PRK_UserClaims].[forRef]'
                        WHEN 'forRef DESC' THEN '[PRK_UserClaims].[forRef] DESC'
