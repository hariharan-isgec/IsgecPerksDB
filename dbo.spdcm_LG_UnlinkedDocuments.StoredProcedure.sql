...
GO
CREATE PROCEDURE [dbo].[spdcm_LG_UnlinkedDocuments]
 ...
 ,RevisionNo NVarChar(2) NOT NULL
  ...
  SET @LGSQL = @LGSQL + '[DCM_Documents].[ProjectID]'  SET @LGSQL = @LGSQL + ', [DCM_Documents].[DocumentID]'  SET @LGSQL = @LGSQL + ', MAX([DCM_Documents].[RevisionNo])'  SET @LGSQL = @LGSQL + ' FROM [DCM_Documents] '
  ...
  SET @LGSQL = @LGSQL + '  WHERE [DCM_Documents].[Locked] = 0 '
  IF (@DF_ProjectID > '') 
    SET @LGSQL = @LGSQL + ' AND [DCM_Documents].[ProjectID] = ''' + @DF_ProjectID + ''''
  SET @LGSQL = @LGSQL + '  GROUP BY [DCM_Documents].[ProjectID],[DCM_Documents].[DocumentID] '
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
                        ...
                        ELSE '[DCM_Documents].[ProjectID],[DCM_Documents].[DocumentID]'
                    END
  EXEC (@LGSQL)
