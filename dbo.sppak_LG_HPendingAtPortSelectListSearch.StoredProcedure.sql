...
GO
CREATE PROCEDURE [dbo].[sppak_LG_HPendingAtPortSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 ,PkgNo Int NOT NULL
  ...
  LEFT OUTER JOIN [aspnet_users] AS [aspnet_users7]
   ...
   WHERE [PAK_PO2].[ProjectID] IN (SELECT PROJECTID from PAK_SiteUserProjects Where UserID =@LoginID) 
     AND [PAK_PO2].[PortID] IN (SELECT PortID from PAK_UserPort Where UserID = @LoginID )
     AND [PAK_PkgListH].[StatusID] = 5  
     AND ( 
