...
GO
CREATE PROCEDURE [dbo].[sppakSTCPOLRSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 ,UploadNo Int NOT NULL
  ...
  INSERT INTO #PageIndex (SerialNo, ItemNo, UploadNo)
  SELECT [PAK_POLineRec].[SerialNo], [PAK_POLineRec].[ItemNo], [PAK_POLineRec].[UploadNo] FROM [PAK_POLineRec]
 ...
 WHERE  
   ( 
         STR(ISNULL([PAK_POLineRec].[SerialNo], 0)) LIKE @KeyWord1
     OR STR(ISNULL([PAK_POLineRec].[ItemNo], 0)) LIKE @KeyWord1
     OR STR(ISNULL([PAK_POLineRec].[UploadNo], 0)) LIKE @KeyWord1
     OR STR(ISNULL([PAK_POLineRec].[DocumentCategoryID], 0)) LIKE @KeyWord1
     OR LOWER(ISNULL([PAK_POLineRec].[UploadRemarks],'')) LIKE @KeyWord1
     OR STR(ISNULL([PAK_POLineRec].[UploadStatusID], 0)) LIKE @KeyWord1
     OR LOWER(ISNULL([PAK_POLineRec].[RevisionNo],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([PAK_POLineRec].[CreatedBy],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([PAK_POLineRec].[ReceiptNo],'')) LIKE @KeyWord1
   ) 
  ORDER BY
