...
GO
CREATE PROCEDURE [dbo].[sppmtTaxiBookingSelectListFilteres]
 ...
 ,BookingID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '    ON [PMT_TaxiBooking].[BookedBy] = [HRM_Employees2].[CardNo]'
  ...
  IF (@Filter_RequestedBy > '') 
    SET @LGSQL = @LGSQL + ' AND [PMT_TaxiBooking].[RequestedBy] = ''' + @Filter_RequestedBy + ''''
  IF (@Filter_VendorID > '') 
    SET @LGSQL = @LGSQL + ' AND [PMT_TaxiBooking].[VendorID] = ''' + @Filter_VendorID + ''''
  IF (@Filter_StatusID > 0) 
    SET @LGSQL = @LGSQL + ' AND [PMT_TaxiBooking].[StatusID] = ' + STR(@Filter_StatusID)
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
                        ...
                        WHEN 'TaxiReportingTime' THEN '[PMT_TaxiBooking].[TaxiReportingTime]'
                        WHEN 'TaxiReportingTime DESC' THEN '[PMT_TaxiBooking].[TaxiReportingTime] DESC'
                        ...
                        WHEN 'BookedBy' THEN '[PMT_TaxiBooking].[BookedBy]'
                        WHEN 'BookedBy DESC' THEN '[PMT_TaxiBooking].[BookedBy] DESC'
                        ...
                        WHEN 'BillPosted' THEN '[PMT_TaxiBooking].[BillPosted]'
                        WHEN 'BillPosted DESC' THEN '[PMT_TaxiBooking].[BillPosted] DESC'
                        WHEN 'CQReceived' THEN '[PMT_TaxiBooking].[CQReceived]'
                        WHEN 'CQReceived DESC' THEN '[PMT_TaxiBooking].[CQReceived] DESC'
