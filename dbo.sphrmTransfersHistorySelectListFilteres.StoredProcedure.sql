...
GO
CREATE PROCEDURE [dbo].[sphrmTransfersHistorySelectListFilteres]
  @StartRowIndex int,
  @MaximumRows int,
  @Executed Bit,
  @Cancelled Bit,
  ...
  SET @LGSQL = @LGSQL + '  WHERE (([HRM_Transfers].[OfficeID]=6 AND [HRM_Transfers].[VerifiedBy] IS NOT NULL) OR ([HRM_Transfers].[OfficeID] <> 6 AND [HRM_Transfers].[VerifiedBy] IS NULL )) '
