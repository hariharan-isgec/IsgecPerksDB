...
GO
CREATE PROCEDURE [dbo].[spnprkApplicationsSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 ,ClaimID Int NOT NULL
  ...
  INSERT INTO #PageIndex (ApplicationID, ClaimID)
  SELECT [PRK_Applications].[ApplicationID], [PRK_Applications].[ClaimID] FROM [PRK_Applications]
 ...
 WHERE  
   ...
   AND ( 
         STR(ISNULL([PRK_Applications].[PerkID], 0)) LIKE @KeyWord1
     ...
     OR STR(ISNULL([PRK_Applications].[ClaimID], 0)) LIKE @KeyWord1
  ...
  FROM [PRK_Applications] 
      INNER JOIN #PageIndex
          ...
          AND [PRK_Applications].[ClaimID] = #PageIndex.ClaimID
  INNER JOIN [PRK_Employees] AS [PRK_Employees1]
    ON [PRK_Applications].[EmployeeID] = [PRK_Employees1].[EmployeeID]
  LEFT OUTER JOIN [PRK_Employees] AS [PRK_Employees2]
    ON [PRK_Applications].[ApprovedBy] = [PRK_Employees2].[EmployeeID]
