...
GO
CREATE PROCEDURE [dbo].[spERP_HK_GetNotesData]
  ...
  BEGIN
  Select 
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@cf,103) and convert(datetime,@ct,103)
          AND NotesHandle LIKE '%P_PURCHASEREQUISITION_200%' AND IndexValue LIKE 'R101%') as  BoilerCMNPR,
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@lf,103) and convert(datetime,@lt,103)
          AND NotesHandle LIKE '%P_PURCHASEREQUISITION_200%'  AND IndexValue LIKE 'R101%') as  BoilerLMNPR,
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@tf,103) and convert(datetime,@tt,103)
  ...
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@cf,103) and convert(datetime,@ct,103)
          AND NotesHandle LIKE '%P_PURCHASEORDER_200%' AND IndexValue LIKE 'P101%') as  BoilerCMNPO,
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@lf,103) and convert(datetime,@lt,103)
          AND NotesHandle LIKE '%P_PURCHASEORDER_200%'  AND IndexValue LIKE 'P101%') as  BoilerLMNPO,
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@tf,103) and convert(datetime,@tt,103)
         AND NotesHandle LIKE '%P_PURCHASEORDER_200%' AND IndexValue LIKE 'P101%') as  BoilerLLNPO,
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@cf,103) and convert(datetime,@ct,103)
          AND NotesHandle LIKE '%P_PURCHASEORDER_200%' AND IndexValue LIKE 'P201%') as  SMDCMNPO,
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@lf,103) and convert(datetime,@lt,103)
          AND NotesHandle LIKE '%P_PURCHASEORDER_200%' AND IndexValue LIKE 'P201%') as  SMDLMNPO,
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@tf,103) and convert(datetime,@tt,103)
          AND NotesHandle LIKE '%P_PURCHASEORDER_200%' AND IndexValue LIKE 'P201%') as  SMDLLNPO,
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@cf,103) and convert(datetime,@ct,103)
          AND NotesHandle LIKE '%P_PURCHASEORDER_200%' AND IndexValue LIKE 'P301%') as  EPCCMNPO,
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@lf,103) and convert(datetime,@lt,103)
          AND NotesHandle LIKE '%P_PURCHASEORDER_200%' AND IndexValue LIKE 'P301%') as  EPCLMNPO,
  (select Count(*) from Note_Notes where Created_Date between convert(datetime,@tf,103) and convert(datetime,@tt,103)
          AND NotesHandle LIKE '%P_PURCHASEORDER_200%' AND IndexValue LIKE 'P301%') as  EPCLLNPO,
