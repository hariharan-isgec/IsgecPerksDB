...
GO
CREATE PROCEDURE [dbo].[spqcm_LG_RequestAllotmentSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 WHERE [QCM_Requests].[CreatedBy] IN (SELECT QCM_EmployeeGroupUsers_1.CardNo FROM QCM_EmployeeGroupUsers INNER JOIN QCM_EmployeeGroupUsers AS QCM_EmployeeGroupUsers_1 ON QCM_EmployeeGroupUsers.GroupiD = QCM_EmployeeGroupUsers_1.GroupiD WHERE QCM_EmployeeGroupUsers.CardNo = @LOGINID )
    AND  [QCM_Requests].[RequestStateID] IN ('UNDERALLOT','ALLOTED','REALLOTED')
   AND ( 
