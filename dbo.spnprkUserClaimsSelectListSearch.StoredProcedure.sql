...
GO
CREATE PROCEDURE [dbo].[spnprkUserClaimsSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 WHERE  
  ...
  AND [PRK_UserClaims].[FinYear] = (@FinYear)
   AND ( 
     ...
     OR LOWER(ISNULL([PRK_UserClaims].[ClaimRefNo],'')) LIKE @KeyWord1
     OR STR(ISNULL([PRK_UserClaims].[FinYear], 0)) LIKE @KeyWord1
     OR STR(ISNULL([PRK_UserClaims].[forRef], 0)) LIKE @KeyWord1
   ) 
  ORDER BY
