...
GO
CREATE PROCEDURE [dbo].[spvr_LG_RequestExecutionSelectListFilteres]
 ...
 ,SRNNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [VR_RequestExecution].[ArrangedBy] IN (SELECT UserID from VR_UserGroup WHERE GroupID IN (Select GroupID from VR_UserGroup WHERE UserID = ''' + @LoginID + '''))'   
