...
GO
CREATE PROCEDURE [dbo].[spidm_LG_UnlinkedDocumentsFilteres]
 ...
 ,RevisionNo NVarChar(2) NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  AND  [IDM_Documents].[ReceivedLocked] = 0 '
  SET @LGSQL = @LGSQL + '  AND  (([IDM_Documents].[VendorDocument] = 0) OR ([IDM_Documents].[VendorDocument] = 1 AND [IDM_Documents].[VDReceived] = 1)) '
  SET @LGSQL = @LGSQL + '  AND  [IDM_Documents].[cmba] NOT IN (SELECT cmba FROM [IDM_TransmittalDetail] WHERE [IDM_TransmittalDetail].[TmtlID] = ' + STR(@Filter_TmtlID) + ') '
  IF (@Filter_ProjectID > '') 
