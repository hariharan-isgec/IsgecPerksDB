...
GO
CREATE PROCEDURE [dbo].[spadmRegisterComplaintSelectListFilteres]
 ...
 ,CallID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '    ON [ADM_Complaints].[AssignedTo] = [aspnet_users4].[UserName]'
  ...
  IF (@Filter_AssignedTo > '') 
    SET @LGSQL = @LGSQL + ' AND [ADM_Complaints].[AssignedTo] = ''' + @Filter_AssignedTo + ''''
  IF (@Filter_CallStatusID > '') 
  BEGIN
		IF (@Filter_CallStatusID = 'NOTCLOSED')
			SET @LGSQL = @LGSQL + ' AND [ADM_Complaints].[CallStatusID] <> ''' + 'CLOSED' + ''''
    ELSE
