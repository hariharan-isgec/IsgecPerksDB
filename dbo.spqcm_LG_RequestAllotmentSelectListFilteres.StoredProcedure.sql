...
GO
CREATE PROCEDURE [dbo].[spqcm_LG_RequestAllotmentSelectListFilteres]
  @Filter_ProjectID NVarChar(6),
  @Filter_AllotedTo NVarChar(8),
 ...
 ,RequestID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [QCM_Requests].[CreatedBy] IN (SELECT QCM_EmployeeGroupUsers_1.CardNo FROM QCM_EmployeeGroupUsers INNER JOIN QCM_EmployeeGroupUsers AS QCM_EmployeeGroupUsers_1 ON QCM_EmployeeGroupUsers.GroupiD = QCM_EmployeeGroupUsers_1.GroupiD WHERE QCM_EmployeeGroupUsers.CardNo =''' + @LOGINID + ''') '
  SET @LGSQL = @LGSQL + ' AND [QCM_Requests].[RequestStateID] IN (''UNDERALLOT'',''ALLOTED'',''REALLOTED'')'
  IF (@Filter_ProjectID > '') 
    SET @LGSQL = @LGSQL + ' AND [QCM_Requests].[ProjectID] = ''' + @Filter_ProjectID + ''''
  IF (@Filter_AllotedTo > '') 
    SET @LGSQL = @LGSQL + ' AND [QCM_Requests].[AllotedTo] = ''' + @Filter_AllotedTo + ''''
  ...
  IF (@Filter_RegionID > '') 
    SET @LGSQL = @LGSQL + ' AND [QCM_Requests].[RegionID] = ''' + @Filter_RegionID + ''''
