...
GO
CREATE PROCEDURE [dbo].[spidm_LG_ReceiveTDSelectListSearch]
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
  @OrderBy NVarChar(50),
 ...
 WHERE [IDM_TransmittalDetail].[DocumentStatusID] =  3 
     AND ( 
  ...
  LEFT OUTER JOIN [IDM_Vendors] AS [IDM_Vendors13]
    ON [IDM_TransmittalDetail].[VendorID] = [IDM_Vendors13].[VendorID]
  WHERE [IDM_TransmittalDetail].[ProjectID] IN (SELECT PROJECTID FROM IDM_ProjectsByEmployee WHERE CardNo = @LoginID) 
	 AND  [IDM_TransmittalDetail].[DocumentStatusID] =  3
   AND ( 
         STR(ISNULL([IDM_TransmittalDetail].[TmtlID], 0)) LIKE @KeyWord1
     OR LOWER(ISNULL([IDM_TransmittalDetail].[ProjectID],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([IDM_TransmittalDetail].[DocumentID],'')) LIKE @KeyWord1
