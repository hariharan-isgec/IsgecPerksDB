...
GO
CREATE PROCEDURE [dbo].[sppakIQCListHAutoCompleteList]
  @LoginID NVarChar(8),
  @Prefix NVarChar(250),
  @Records Int,
  @ByCode Int 
  ...
  SET @LGSQL = @LGSQL + ' AND (LTRIM(STR(ISNULL([PAK_QCListH].[SerialNo],0))) LIKE ''' + @Prefix1 + ''''
  SET @LGSQL = @LGSQL + ' OR LTRIM(STR(ISNULL([PAK_QCListH].[QCLNo],0))) LIKE ''' + @Prefix1 + ''''
