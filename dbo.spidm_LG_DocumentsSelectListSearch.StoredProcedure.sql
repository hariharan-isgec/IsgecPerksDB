...
GO
CREATE PROCEDURE [dbo].[spidm_LG_DocumentsSelectListSearch]
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
  @OrderBy NVarChar(50),
  ...
  IF (@PRFilter = 0)
	 ...
	 WHERE ([IDM_Documents].[RevisionNo] = (SELECT MAX(RevisionNo) AS TEMP1 FROM IDM_Documents AS B WHERE ([B].[ProjectID] = [IDM_Documents].[ProjectID]) AND ([B].[DocumentID] = [IDM_Documents].[DocumentID])))  
		   AND ( 
    ...
    ELSE
			...
			LEFT OUTER JOIN [IDM_WBS] AS [IDM_WBS11]
				ON [IDM_Documents].[WBSID] = [IDM_WBS11].[WBSID]
		 WHERE  ([IDM_Documents].[RevisionNo] = (SELECT MAX(RevisionNo) AS TEMP1 FROM IDM_Documents AS B WHERE ([B].[ProjectID] = [IDM_Documents].[ProjectID]) AND ([B].[DocumentID] = [IDM_Documents].[DocumentID])))  
         AND [IDM_Documents].[ProjectID] IN (SELECT PROJECTID FROM IDM_ProjectsByEmployee WHERE CardNo = @LoginID) 
				 AND ( 
						 LOWER(ISNULL([IDM_Documents].[ProjectID],'')) LIKE @KeyWord1
				 OR LOWER(ISNULL([IDM_Documents].[DocumentID],'')) LIKE @KeyWord1
				 OR LOWER(ISNULL([IDM_Documents].[RevisionNo],'')) LIKE @KeyWord1
