...
GO
CREATE PROCEDURE [dbo].[spadmPendingProblemsSelectListSearch]
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 ,SerialNo Int NOT NULL
  ...
  LEFT OUTER JOIN [HRM_Employees] AS [HRM_Employees8]
    ON [ADM_ServiceSheetDetails].[ClosedBy] = [HRM_Employees8].[CardNo]
 WHERE  
      [ADM_ServiceSheetDetails].[ProblemIdentified] = @ProblemIdentified
  AND [ADM_ServiceSheetDetails].[ProblemClosed] = @ProblemClosed
   AND ( 
     ...
     OR LOWER(ISNULL([HRM_Employees8].[CardNo],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([HRM_Employees8].[EmployeeName],'')) LIKE @KeyWord1
     OR STR(ISNULL([HRM_Employees8].[C_OfficeID], 0)) LIKE @KeyWord1
     OR LOWER(ISNULL([HRM_Employees8].[C_DepartmentID],'')) LIKE @KeyWord1
     OR STR(ISNULL([HRM_Employees8].[C_DesignationID], 0)) LIKE @KeyWord1
   ) 
  ORDER BY
     ...
     CASE @orderBy WHEN 'HRM_Employees8_CardNo' THEN [HRM_Employees8].[CardNo] END,
     CASE @orderBy WHEN 'HRM_Employees8_CardNo DESC' THEN [HRM_Employees8].[CardNo] END DESC,
     ...
     CASE @orderBy WHEN 'HRM_Employees8_C_OfficeID' THEN [HRM_Employees8].[C_OfficeID] END,
     CASE @orderBy WHEN 'HRM_Employees8_C_OfficeID DESC' THEN [HRM_Employees8].[C_OfficeID] END DESC 
  ...
  SELECT
		...
		[HRM_Employees8].[CardNo] AS HRM_Employees8_CardNo,
		[HRM_Employees8].[EmployeeName] AS HRM_Employees8_EmployeeName,
		[HRM_Employees8].[C_OfficeID] AS HRM_Employees8_C_OfficeID 
