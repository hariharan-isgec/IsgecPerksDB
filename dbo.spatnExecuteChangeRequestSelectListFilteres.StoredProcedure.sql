...
GO
CREATE PROCEDURE [dbo].[spatnExecuteChangeRequestSelectListFilteres]
  ...
  SET @LGSQL = @LGSQL + '  WHERE [ATN_ApproverChangeRequest].[Executed] = 0 AND YEAR([RequestedOn]) = YEAR(GetDate())'
  IF (@Filter_UserID > '') 
    SET @LGSQL = @LGSQL + ' AND [ATN_ApproverChangeRequest].[UserID] = ''' + @Filter_UserID + ''''
