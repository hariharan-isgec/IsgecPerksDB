...
GO
CREATE Procedure [dbo].[spatn_LG_AttendanceUnderApproval]
  ...
  SELECT
    ...
    [ATN_ApplHeader4].[ApprovedBy] AS ATN_ApplHeader4_ApprovedBy,
  ...
  LEFT OUTER JOIN [HRM_Employees] AS [HRM_Employees5]
    ON [ATN_ApplHeader4].[ApprovedBy] = [HRM_Employees5].[CardNo]
  WHERE
        [ATN_Attendance].[FinYear] = @FinYear  
    AND [ATN_Attendance].[ApplStatusID] = @ApplStatusID  
    AND [ATN_ApplHeader4].[ApprovedBy] = @ApprovedBy 
