...
GO
CREATE PROCEDURE [dbo].[spqcm_LG_RequestsSelectByCreatedByMonthYear]
	@ForMonth Int,
	@ForYear Int,
  @LoginID NVarChar(8),
  ...
  WHERE
  [QCM_Requests].[AllotedTo] IN 
  (SELECT QCM_InspectorGroupUsers.CardNo FROM QCM_EmployeeGroupUsers 
    INNER JOIN QCM_InspectorGroupEmployeeGroup ON QCM_EmployeeGroupUsers.GroupiD = QCM_InspectorGroupEmployeeGroup.EmployeeGroupID 
	INNER JOIN QCM_InspectorGroupUsers ON QCM_InspectorGroupEmployeeGroup.InspectorGroupID = QCM_InspectorGroupUsers.GroupID
	WHERE     (QCM_EmployeeGroupUsers.CardNo = @LOGINID ))
  AND (YEAR([QCM_Requests].[AllotedStartDate]) = @ForYear AND (MONTH([QCM_Requests].[AllotedStartDate]) = @ForMonth OR MONTH([QCM_Requests].[AllotedFinishDate]) = @ForMonth ))
  ORDER BY [QCM_Requests].[AllotedTo], [QCM_Requests].[AllotedStartDate]
