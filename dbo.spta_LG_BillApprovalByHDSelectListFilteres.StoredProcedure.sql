...
GO
CREATE Procedure [dbo].[spta_LG_BillApprovalByHDSelectListFilteres]
  @Filter_TABillNo Int,
 ...
 ,TABillNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  AND [TA_Bills].[EmployeeID] IN (select cardno from hrm_employees where taVerifier = ''' + @LoginID + ''')'
  ...
  ELSE
    SET @LGSQL = @LGSQL + ' AND [TA_Bills].[ApprovedBy] = ''' + @LoginID + ''''  
