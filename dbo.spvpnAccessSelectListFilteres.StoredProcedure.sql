...
GO
CREATE Procedure [dbo].[spvpnAccessSelectListFilteres]
 ...
 ,RequestID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + ' AND [VPN_ConnectivityAccess].[Removed] = ''' + CONVERT(NVarChar(10), @Removed) + ''''
  IF (@Filter_CardNo > '') 
    SET @LGSQL = @LGSQL + ' AND [VPN_ConnectivityAccess].[CardNo] = ''' + @Filter_CardNo + ''''
  IF (@Filter_C_OfficeID > 0) 
    SET @LGSQL = @LGSQL + ' AND [VPN_ConnectivityAccess].[C_OfficeID] = ' + STR(@Filter_C_OfficeID)
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
                        ...
                        WHEN 'BaaNPW' THEN '[VPN_ConnectivityAccess].[BaaNPW]'
                        WHEN 'BaaNPW DESC' THEN '[VPN_ConnectivityAccess].[BaaNPW] DESC'
                        WHEN 'PLMpw' THEN '[VPN_ConnectivityAccess].[PLMpw]'
                        WHEN 'PLMpw DESC' THEN '[VPN_ConnectivityAccess].[PLMpw] DESC'
                        WHEN 'EMailPW' THEN '[VPN_ConnectivityAccess].[EMailPW]'
                        WHEN 'EMailPW DESC' THEN '[VPN_ConnectivityAccess].[EMailPW] DESC'
                        WHEN 'IsLaptop' THEN '[VPN_ConnectivityAccess].[IsLaptop]'
                        WHEN 'IsLaptop DESC' THEN '[VPN_ConnectivityAccess].[IsLaptop] DESC'
                        WHEN 'ConfidentialData' THEN '[VPN_ConnectivityAccess].[ConfidentialData]'
                        WHEN 'ConfidentialData DESC' THEN '[VPN_ConnectivityAccess].[ConfidentialData] DESC'
                        WHEN 'SensitiveData' THEN '[VPN_ConnectivityAccess].[SensitiveData]'
                        WHEN 'SensitiveData DESC' THEN '[VPN_ConnectivityAccess].[SensitiveData] DESC'
                        WHEN 'InternetAccess' THEN '[VPN_ConnectivityAccess].[InternetAccess]'
                        WHEN 'InternetAccess DESC' THEN '[VPN_ConnectivityAccess].[InternetAccess] DESC'
                        WHEN 'ExternalEMail' THEN '[VPN_ConnectivityAccess].[ExternalEMail]'
                        WHEN 'ExternalEMail DESC' THEN '[VPN_ConnectivityAccess].[ExternalEMail] DESC'
