...
GO
CREATE PROCEDURE [dbo].[sppakTCPOLRDAutoCompleteList]
  @LoginID NVarChar(8),
  @Prefix NVarChar(250),
  @Records Int,
  @ByCode Int 
  ...
  SET @LGSQL = @LGSQL + ' [PAK_POLineRecDoc].[DocumentID]' 
  ...
  SET @LGSQL = @LGSQL + ' OR LOWER(ISNULL([PAK_POLineRecDoc].[DocumentID],'''')) LIKE ''' + @Prefix1 + ''''
  SET @LGSQL = @LGSQL + ')' 
