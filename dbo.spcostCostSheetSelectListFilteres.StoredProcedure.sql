...
GO
CREATE PROCEDURE [dbo].[spcostCostSheetSelectListFilteres]
 ...
 ,Revision Int NOT NULL
  ...
  IF (@Filter_FinYear > 0) 
    SET @LGSQL = @LGSQL + ' AND [COST_CostSheet].[FinYear] = ' + STR(@Filter_FinYear)
  IF (@Filter_Quarter > 0) 
    SET @LGSQL = @LGSQL + ' AND [COST_CostSheet].[Quarter] = ' + STR(@Filter_Quarter)
  IF (@Filter_StatusID > 0) 
    SET @LGSQL = @LGSQL + ' AND [COST_CostSheet].[StatusID] = ' + STR(@Filter_StatusID)
  IF (@Filter_CreatedBy > '') 
    SET @LGSQL = @LGSQL + ' AND [COST_CostSheet].[CreatedBy] = ''' + @Filter_CreatedBy + ''''
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
                        ...
                        ELSE '[COST_CostSheet].[ProjectGroupID],[COST_CostSheet].[FinYear],[COST_CostSheet].[Quarter],[COST_CostSheet].[Revision]'
                    END
  EXEC (@LGSQL)
