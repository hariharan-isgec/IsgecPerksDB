...
GO
CREATE Procedure [dbo].[spspmtFESelectListFilteres]
  @Filter_RequestedBy NVarChar(8),
  @Filter_VendorID NVarChar(6),
  @Filter_PlaceToBeVisited NVarChar(30),
  @StartRowIndex int,
  @MaximumRows int,
 ...
 ,FEID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + ' AND [SPMT_FE].[CreatedBy] = (''' + @CreatedBy + ''')'
  IF (@Filter_RequestedBy > '') 
    SET @LGSQL = @LGSQL + ' AND [SPMT_FE].[RequestedBy] = ''' + @Filter_RequestedBy + ''''
  IF (@Filter_VendorID > '') 
    SET @LGSQL = @LGSQL + ' AND [SPMT_FE].[VendorID] = ''' + @Filter_VendorID + ''''
  IF (@Filter_PlaceToBeVisited > '') 
    SET @LGSQL = @LGSQL + ' AND [SPMT_FE].[PlaceToBeVisited] = ''' + @Filter_PlaceToBeVisited + ''''
