...
GO
create PROCEDURE [dbo].[spidm_LG_TSelectListFilteres]
 ...
 ,TmtlID Int NOT NULL
  ...
  IF (@PRFilter = 1) 
    SET @LGSQL = @LGSQL + ' AND [IDM_Transmittal].[ProjectID] IN (SELECT PROJECTID FROM IDM_ProjectsByEmployee WHERE CardNo = ''' + @LoginID + ''') '
  ...
  IF (@Filter_CustomerID > '') 
    SET @LGSQL = @LGSQL + ' AND [IDM_Transmittal].[CustomerID] = ''' + @Filter_CustomerID + ''''
  IF (@Filter_VendorID > '') 
