...
GO
CREATE PROCEDURE [dbo].[spidmEmployeesSelectListFilteres]
 ...
 ,CardNo NVarChar(8) NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  LEFT OUTER JOIN [IDM_ResponsibleAgency] AS [IDM_ResponsibleAgency5]'
  SET @LGSQL = @LGSQL + '    ON [HRM_Employees].[ResponsibleAgencyID] = [IDM_ResponsibleAgency5].[ResponsibleAgencyID]'
  ...
  IF (@Filter_ResponsibleAgencyID > 0) 
    SET @LGSQL = @LGSQL + ' AND [HRM_Employees].[ResponsibleAgencyID] = ' + STR(@Filter_ResponsibleAgencyID)
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
                        ...
                        WHEN 'HRM_Departments2_Description' THEN '[HRM_Departments].[Description]'
                        WHEN 'HRM_Departments2_Description DESC' THEN '[HRM_Departments2].[Description] DESC'
                        WHEN 'HRM_Designations3_Description' THEN '[HRM_Designations].[Description]'
                        WHEN 'HRM_Designations3_Description DESC' THEN '[HRM_Designations3].[Description] DESC'
                        WHEN 'HRM_Offices1_Description' THEN '[HRM_Offices].[Description]'
                        WHEN 'HRM_Offices1_Description DESC' THEN '[HRM_Offices1].[Description] DESC'
                        WHEN 'IDM_ResponsibleAgency5_Description' THEN '[IDM_ResponsibleAgency].[Description]'
                        WHEN 'IDM_ResponsibleAgency5_Description DESC' THEN '[IDM_ResponsibleAgency5].[Description] DESC'
                        WHEN 'IDM_Projects4_Description' THEN '[IDM_Projects].[Description]'
