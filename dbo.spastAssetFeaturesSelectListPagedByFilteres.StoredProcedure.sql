...
GO
CREATE PROCEDURE [dbo].[spastAssetFeaturesSelectListPagedByFilteres]
  @Filter_MrnID Int,
  ...
  SET @LGSQL = @LGSQL + '    ON [AST_AssetDetails].[UserID] = [PRK_Employees4].[EmployeeID]'
  SET @LGSQL = @LGSQL + '  LEFT OUTER JOIN [AST_Locations] AS [AST_Locations5]'
  SET @LGSQL = @LGSQL + '    ON [AST_AssetDetails].[LocationID] = [AST_Locations5].[LocationID]'
  ...
  IF (@Filter_MrnID > 0) 
    SET @LGSQL = @LGSQL + ' AND [AST_AssetDetails].[MrnID] = ' + STR(@Filter_MrnID)
