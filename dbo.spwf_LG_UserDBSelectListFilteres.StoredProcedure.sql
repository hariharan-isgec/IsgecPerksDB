...
GO
CREATE PROCEDURE [dbo].[spwf_LG_UserDBSelectListFilteres]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @OrderBy NVarChar(50),
 ...
 ,DashboardID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [WF_Dashboards].[InitializedByMe] = 0 '
  SET @LGSQL = @LGSQL + '  AND [WF_Dashboards].[Active] = 1 '
  SET @LGSQL = @LGSQL + '  AND [WF_Dashboards].[DashboardID] IN (select DashboardID from wf_userDashboards where active=1 and userid=''' + @LoginID + ''' )'
