...
GO
CREATE PROCEDURE [dbo].[spqapRequestsApproveSelectListFilteres]
  @Filter_DocumentTypeID Int,
  @Filter_ProjectID NVarChar(6),
  @Filter_SupplierID NVarChar(9),
  @Filter_CreatersDivision NVarChar(6),
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @StatusID Int,
 ...
 ,RequestNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + ' AND [QAP_Requests].[StatusID] IN (3,4)  ' 
  ...
  IF (@Filter_CreatersDivision > '') 
    SET @LGSQL = @LGSQL + ' AND [QAP_Requests].[CreatersDivision] = ''' + @Filter_CreatersDivision + ''''
