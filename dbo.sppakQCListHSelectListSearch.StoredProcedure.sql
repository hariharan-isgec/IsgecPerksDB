...
GO
CREATE PROCEDURE [dbo].[sppakQCListHSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 ,SerialNo Int NOT NULL
  ...
  INSERT INTO #PageIndex (QCLNo, SerialNo)
  SELECT [PAK_QCListH].[QCLNo], [PAK_QCListH].[SerialNo] FROM [PAK_QCListH]
 ...
 WHERE  
   ( 
         STR(ISNULL([PAK_QCListH].[QCLNo], 0)) LIKE @KeyWord1
     ...
     OR STR(ISNULL([PAK_QCListH].[SerialNo], 0)) LIKE @KeyWord1
  ...
  FROM [PAK_QCListH] 
      INNER JOIN #PageIndex
          ON [PAK_QCListH].[QCLNo] = #PageIndex.QCLNo
          AND [PAK_QCListH].[SerialNo] = #PageIndex.SerialNo
  LEFT OUTER JOIN [aspnet_users] AS [aspnet_users1]
    ON [PAK_QCListH].[CreatedBy] = [aspnet_users1].[LoginID]
  INNER JOIN [PAK_PO] AS [PAK_PO2]
    ON [PAK_QCListH].[SerialNo] = [PAK_PO2].[SerialNo]
