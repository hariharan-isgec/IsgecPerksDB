...
GO
CREATE PROCEDURE [dbo].[spspmtNewUnLinkedSBHSelectListFilteres]
 ...
 ,IRNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [SPMT_newSBH].[AdviceNo] IS NULL '
  ...
  IF (@Filter_SupplierName > '') 
    SET @LGSQL = @LGSQL + ' AND [SPMT_newSBH].[SupplierName] = ''' + @Filter_SupplierName + ''''
