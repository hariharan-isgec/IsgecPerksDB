...
GO
CREATE PROCEDURE [dbo].[spcalAssignComplaintsSelectListFilteres]
  @Filter_UserID NVarChar(8),
  @Filter_CallTypeID Int,
  @Filter_AttenderID NVarChar(8),
  ...
  IF (@Filter_AttenderID > '') 
    SET @LGSQL = @LGSQL + ' AND [CAL_Register].[AttenderID] = ''' + @Filter_AttenderID + ''''
