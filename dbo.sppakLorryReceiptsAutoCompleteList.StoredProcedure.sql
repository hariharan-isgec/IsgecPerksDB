...
GO
CREATE PROCEDURE [dbo].[sppakLorryReceiptsAutoCompleteList]
  @LoginID NVarChar(8),
  @Prefix NVarChar(250),
  @Records Int,
  @ByCode Int 
  ...
  SET @LGSQL = @LGSQL + ' [VR_LorryReceipts].[TransporterName]' 
  ...
  SET @LGSQL = @LGSQL + ' OR LOWER(ISNULL([VR_LorryReceipts].[TransporterName],'''')) LIKE ''' + @Prefix1 + ''''
  SET @LGSQL = @LGSQL + ')' 
