...
GO
CREATE PROCEDURE [dbo].[spspmtSupplierBillSelectListFilteres]
 ...
 ,IRNo Int NOT NULL
  ...
  IF (@Filter_PurchaseType > '') 
    SET @LGSQL = @LGSQL + ' AND [SPMT_SupplierBill].[PurchaseType] = ''' + @Filter_PurchaseType + ''''
  IF (@Filter_IRNo > 0) 
    SET @LGSQL = @LGSQL + ' AND [SPMT_SupplierBill].[IRNo] = ' + STR(@Filter_IRNo)
