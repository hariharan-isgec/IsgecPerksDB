...
GO
CREATE PROCEDURE [dbo].[sppak_LG_PkgPortHSelectListFilteres]
  @Filter_SerialNo Int,
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
 ...
 ,PkgNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [PAK_PkgListH].[PortID] IN (Select PortID from PAK_UserPort where UserID =''' + @LoginID + ''' ) '
  SET @LGSQL = @LGSQL + '    AND [PAK_PkgListH].[SerialNo] = 0' 
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
                        ...
                        ELSE '[PAK_PkgListH].[SerialNo],[PAK_PkgListH].[PkgNo] DESC'
                    END
  EXEC (@LGSQL)
