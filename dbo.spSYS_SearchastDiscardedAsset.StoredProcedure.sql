...
GO
CREATE PROCEDURE [dbo].[spSYS_SearchastDiscardedAsset]
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
  ...
  WHERE STR(ISNULL([AST_AssetDetails].[AssetID], 0)) LIKE @KeyWord1
     ...
     OR LOWER(ISNULL([AST_AssetDetails].[Remarks],'')) LIKE @KeyWord1
     ...
     OR LOWER(ISNULL([AST_Locations4].[Descriptions],'')) LIKE @KeyWord1
  ...
  + ',' + ISNULL([AST_AssetDetails].[Remarks],'') 
  ...
  + ',' + ISNULL([AST_Locations4].[Descriptions],'') 
