...
GO
CREATE PROCEDURE [dbo].[spqcm_LG_RequestsBySupplierSelectList]
 ...
 ,RequestID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  LEFT OUTER JOIN [IDM_Vendors] AS [IDM_Vendors7]'
  ...
  IF (SubString(@LoginID,1,2) = 'UP') 
    SET @LGSQL = @LGSQL + '  AND [QCM_Requests].[CreatedBy] = ''' + @LoginID + ''''
  --ELSE
  --  SET @LGSQL = @LGSQL + '  AND SubString([QCM_Requests].[CreatedBy],1,2) = ''UP'''
  IF (@ProjectID > '') 
    SET @LGSQL = @LGSQL + ' AND [QCM_Requests].[ProjectID] = ''' + @ProjectID + ''''
  IF (@SearchState = 1 )
    BEGIN
		SET @LGSQL = @LGSQL + '  AND ( '
		SET @LGSQL = @LGSQL + '  	 STR(ISNULL([QCM_Requests].[RequestID], 0)) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR LOWER(ISNULL([QCM_Requests].[ProjectID],'''')) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR LOWER(ISNULL([QCM_Requests].[OrderNo],'''')) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR LOWER(ISNULL([QCM_Requests].[Description],'''')) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR LOWER(ISNULL([QCM_Requests].[TotalRequestedQuantity],'')) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR LOWER(ISNULL([QCM_Requests].[CreationRemarks],'''')) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR LOWER(ISNULL([QCM_Requests].[RequestStateID],'''')) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR STR(ISNULL([QCM_Requests].[InspectionStageiD], 0)) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR LOWER(ISNULL([QCM_Requests].[AllotedTo],'''')) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR LOWER(ISNULL([QCM_Requests].[AllotedBy],'''')) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR LOWER(ISNULL([QCM_Requests].[AllotmentRemarks],'''')) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR STR(ISNULL([QCM_Requests].[InspectionStatusID], 0)) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR LOWER(ISNULL([QCM_Requests].[CancelledBy],'''')) LIKE ''' + @KeyWord1 + ''''
		SET @LGSQL = @LGSQL + '   OR LOWER(ISNULL([QCM_Requests].[CancellationRemarks],'''')) LIKE ''' + @KeyWord1 + ''''
	    SET @LGSQL = @LGSQL + '  ) '
	END
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
		...
		ELSE '[QCM_Requests].[RequestID] DESC'
     END
  EXEC (@LGSQL)
  ...
  LEFT OUTER JOIN [IDM_Vendors] AS [IDM_Vendors7]
