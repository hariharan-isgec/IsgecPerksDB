...
GO
CREATE PROCEDURE [dbo].[sptarTRBudgetedByPMSelectListFilteres]
 ...
 ,RequestID Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [TA_TravelRequest].[StatusID] >= 6 '
  IF (@LoginID <> '0340') 
    SET @LGSQL = @LGSQL + ' AND [TA_TravelRequest].[ProjectManagerID] = ''' + @LoginID + ''''
  IF (@Filter_ProjectID > '') 
