...
GO
CREATE Procedure [dbo].[sptaDepartmentsSelectListFilteres]
  @Filter_DepartmentHead NVarChar(8),
  @Filter_BusinessHead NVarChar(8),
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @OrderBy NVarChar(50),
 ...
 ,DepartmentID NVarChar(6) NOT NULL
  ...
  SET @LGSQL = @LGSQL + '    ON [HRM_Departments].[BusinessHead] = [HRM_Employees2].[CardNo]'
  SET @LGSQL = @LGSQL + '  LEFT OUTER JOIN [HRM_Employees] AS [HRM_Employees1]'
  SET @LGSQL = @LGSQL + '    ON [HRM_Departments].[DepartmentHead] = [HRM_Employees1].[CardNo]'
  SET @LGSQL = @LGSQL + '  WHERE 1 = 1 '
  IF (@Filter_DepartmentHead > '') 
    SET @LGSQL = @LGSQL + ' AND [HRM_Departments].[DepartmentHead] = ''' + @Filter_DepartmentHead + ''''
  IF (@Filter_BusinessHead > '') 
    SET @LGSQL = @LGSQL + ' AND [HRM_Departments].[BusinessHead] = ''' + @Filter_BusinessHead + ''''
