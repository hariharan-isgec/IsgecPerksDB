...
GO
CREATE PROCEDURE [dbo].[spvr_LG_UnLinkedRequestSelectListFilteres]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @RequestStatus Int,
 ...
 ,RequestNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE ([VR_VehicleRequest].[RequestedBy] IN (SELECT UserID from VR_UserGroup WHERE GroupID IN (Select GroupID from VR_UserGroup WHERE UserID = ''' + @LoginID + ''')) OR (OutOfContract=1 AND OutOfContract=(select top 1 OutOfContract from VR_UserGroup where UserID='''+@LoginID+''') )  )' 
