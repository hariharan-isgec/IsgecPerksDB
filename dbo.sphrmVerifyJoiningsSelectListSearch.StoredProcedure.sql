...
GO
CREATE PROCEDURE [dbo].[sphrmVerifyJoiningsSelectListSearch]
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
  @Freezed Bit,
  ...
  LEFT OUTER JOIN [aspnet_Users] AS [aspnet_Users8]
    ON [HRM_Employees].[ModifiedBy] = [aspnet_Users8].[UserName]
 WHERE  
   ...
   AND ( 
     ...
     OR LOWER(ISNULL([aspnet_Users8].[UserName],'')) LIKE @KeyWord1
     OR LOWER(ISNULL([aspnet_Users8].[UserFullName],'')) LIKE @KeyWord1
   ) 
  ORDER BY
     ...
     CASE @orderBy WHEN 'aspnet_Users8_UserName' THEN [aspnet_Users8].[UserName] END,
     CASE @orderBy WHEN 'aspnet_Users8_UserName DESC' THEN [aspnet_Users8].[UserName] END DESC,
  ...
  SELECT
		...
		[aspnet_Users8].[UserName] AS aspnet_Users8_UserName,
