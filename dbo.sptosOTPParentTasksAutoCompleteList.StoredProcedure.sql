...
GO
CREATE PROCEDURE [dbo].[sptosOTPParentTasksAutoCompleteList]
  ...
  SET @LGSQL = @LGSQL + ' AND (LOWER(ISNULL([TOSv_Parent].[ProjectID],'''')) LIKE ''' + @Prefix1 + ''''
  SET @LGSQL = @LGSQL + ' OR LTRIM(STR(ISNULL([TOSv_Parent].[TaskID],0))) LIKE ''' + @Prefix1 + ''''
