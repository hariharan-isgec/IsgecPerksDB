...
GO
CREATE PROCEDURE [dbo].[spadmPendingProblemsSelectListFilteres]
  @Filter_ServiceID NVarChar(10),
  @Filter_CheckPointID Int,
  @StartRowIndex int,
  @MaximumRows int,
 ...
 ,SerialNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '    ON [ADM_ServiceSheetDetails].[ClosedBy] = [HRM_Employees8].[CardNo]'
  SET @LGSQL = @LGSQL + '  WHERE 1 = 1 '
  IF (@Filter_ServiceID > '') 
    SET @LGSQL = @LGSQL + ' AND [ADM_ServiceSheetDetails].[ServiceID] = ''' + @Filter_ServiceID + ''''
  IF (@Filter_CheckPointID > 0) 
    SET @LGSQL = @LGSQL + ' AND [ADM_ServiceSheetDetails].[CheckPointID] = ' + STR(@Filter_CheckPointID)
  SET @LGSQL = @LGSQL + ' AND [ADM_ServiceSheetDetails].[ProblemIdentified] = ''' + CONVERT(NVarChar(10), @ProblemIdentified) + ''''
  SET @LGSQL = @LGSQL + ' AND [ADM_ServiceSheetDetails].[ProblemClosed] = ''' + CONVERT(NVarChar(10), @ProblemClosed) + ''''
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
                        ...
                        WHEN 'HRM_Employees8_CardNo' THEN '[HRM_Employees8].[CardNo]'
                        WHEN 'HRM_Employees8_CardNo DESC' THEN '[HRM_Employees8].[CardNo] DESC'
                        ...
                        WHEN 'HRM_Employees8_C_OfficeID' THEN '[HRM_Employees8].[C_OfficeID]'
                        WHEN 'HRM_Employees8_C_OfficeID DESC' THEN '[HRM_Employees8].[C_OfficeID] DESC'
