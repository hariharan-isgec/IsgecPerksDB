...
GO
CREATE procedure [dbo].[sptaCHSelectListFilteres]
 ...
 ,TABillNo Int NOT NULL
  ...
  SET @LGSQL = @LGSQL + '  WHERE [TA_Bills].[BillStatusID] IN (2,3,4,5,11,12,13,15,16,17) '
  IF (@Filter_TravelTypeID > 0) 
    SET @LGSQL = @LGSQL + ' AND [TA_Bills].[TravelTypeID] = ' + STR(@Filter_TravelTypeID)
  IF (@Filter_DestinationCity > '') 
    SET @LGSQL = @LGSQL + ' AND [TA_Bills].[TABillNo] = ''' + @Filter_DestinationCity + ''''
  IF (@Filter_ProjectID > '') 
