...
GO
CREATE PROCEDURE [dbo].[sptos_LG_OutlookNotesSelectByTaskID]
  @TaskID Int,
  @ProgressID Int,
  ...
  WHERE [TOS_Notes].[CreatedOn] >= (SELECT CreatedOn FROM TOS_CLPDATA WHERE PROJECTID = @PROJECTID AND PROGRESSID=@PROGRESSID)
     AND [TOS_Notes].[TaskID] = @TaskID  
     AND [TOS_Notes].[NoteSubject] IS NOT NULL 
  ORDER BY [TOS_Notes].[NoteID] DESC
