...
GO
CREATE PROCEDURE [dbo].[sptarTRApprovalSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 WHERE [TA_TravelRequest].[StatusID] >= 7
   AND ([TA_TravelRequest].[RequestedFor] IN (case when @LoginID<>'0340' then (Select CardNo From HRM_Employees where TAApprover = @LoginID) else [TA_TravelRequest].[RequestedFor] end )
        OR [TA_TravelRequest].[ApprovedBy] = @LoginID
       )
   AND ( 
