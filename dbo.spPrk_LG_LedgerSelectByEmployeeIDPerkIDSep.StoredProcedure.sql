...
GO
CREATE PROCEDURE [dbo].[spPrk_LG_LedgerSelectByEmployeeIDPerkIDSep]
  @EmployeeID Int,
  @PerkID Int,
  @TranDate DateTime,
  @FinYear Int,
  ...
  FROM [PRK_Ledger] 
  WHERE [PRK_Ledger].[FinYear] = @FinYear
	AND	[PRK_Ledger].[EmployeeID] = @EmployeeID 
	AND	[PRK_Ledger].[PerkID] = @PerkID 
	AND [PRK_Ledger].[TranDate] < @TranDate
  ORDER BY [PRK_Ledger].[TranDate]
