...
GO
CREATE PROCEDURE [dbo].[spelogChargeCategoriesSelectListSearch]
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
  @KeyWord VarChar(250),
 ...
 ,ChargeCategoryID Int NOT NULL
  ...
  INSERT INTO #PageIndex (ChargeCategoryID)
  SELECT [ELOG_ChargeCategories].[ChargeCategoryID] FROM [ELOG_ChargeCategories]
 ...
 WHERE  
   ( 
         STR(ISNULL([ELOG_ChargeCategories].[ChargeCategoryID], 0)) LIKE @KeyWord1
     OR STR(ISNULL([ELOG_ChargeCategories].[ShipmentModeID], 0)) LIKE @KeyWord1
     OR STR(ISNULL([ELOG_ChargeCategories].[LocationCountryID], 0)) LIKE @KeyWord1
     OR STR(ISNULL([ELOG_ChargeCategories].[CargoTypeID], 0)) LIKE @KeyWord1
     OR STR(ISNULL([ELOG_ChargeCategories].[StuffingTypeID], 0)) LIKE @KeyWord1
     OR STR(ISNULL([ELOG_ChargeCategories].[StuffingPointID], 0)) LIKE @KeyWord1
     OR STR(ISNULL([ELOG_ChargeCategories].[BreakbulkTypeID], 0)) LIKE @KeyWord1
     OR STR(ISNULL([ELOG_ChargeCategories].[ChargeTypeID], 0)) LIKE @KeyWord1
     OR LOWER(ISNULL([ELOG_ChargeCategories].[Description],'')) LIKE @KeyWord1
   ) 
  ORDER BY
