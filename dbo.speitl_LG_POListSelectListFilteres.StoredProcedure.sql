...
GO
CREATE Procedure [dbo].[speitl_LG_POListSelectListFilteres]
...
--PO Status Is Empty or Partial
--Supplier ID = 'S' + LoginUser ID
  SET @LGSQL = @LGSQL + '  WHERE [EITL_POList].[POStatusID] IN (2,3) '
  --SET @LGSQL = @LGSQL + '  AND   [EITL_POList].[SupplierID] = ''S' + @LoginID + '''  '
  IF (@Filter_ProjectID > '') 
