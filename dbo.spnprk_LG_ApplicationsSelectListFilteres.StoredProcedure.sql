...
GO
Create PROCEDURE [dbo].[spnprk_LG_ApplicationsSelectListFilteres]
  @Filter_ClaimID Int,
  @LoginID NVarChar(8),
  @StartRowIndex int,
  @MaximumRows int,
 ...
 ,ClaimID Int NOT NULL
  ...
  IF (@Filter_ClaimID > 0) 
    SET @LGSQL = @LGSQL + ' AND [PRK_Applications].[ClaimID] = ' + STR(@Filter_ClaimID)
  ...
  SET @LGSQL = @LGSQL + CASE @OrderBy
                        ...
                        ELSE '[PRK_Applications].[ApplicationID],[PRK_Applications].[ClaimID]'
                    END
  EXEC (@LGSQL)
